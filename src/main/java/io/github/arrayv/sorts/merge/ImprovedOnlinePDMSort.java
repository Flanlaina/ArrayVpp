...
final public class ImprovedOnlinePDMSort extends Sort {
    ...
    public int merge(int[] array, int[] tmp, int offstmp, int a, int b, int run, int depth, boolean aux) {
    	...
    	Writes.recursion();
    	int l, la = Math.abs(l = merge(array, tmp, offstmp, a, b, run>>1, depth+1, aux)), r, ra;
    	if(l != 0) { // recursion limiter
        	Writes.recursion();
    		ra = Math.abs(r = merge(array, tmp, offstmp, a+la, b, run>>1, depth+1, aux));
    	} else ra = r = 0;
    	int pa = ra, p = r;
		while(la / 4 > ra && pa > 0) { // for halves
    		pa = Math.abs(p = merge(array, tmp, offstmp, a+la+ra, b, la, depth+1, aux));
    		if(pa == 0) break;
			if(ra > pa) {
