...
public final class HalfSort extends Sort {
		...
		for (int i = 0; i <= combineLen && (
			...
			for (tagIndex = 1; tagIndex < blockCount; tagIndex++) {
				...
				for (int rightIndex = tagIndex; rightIndex < blockCount; rightIndex++) {
					int j = Reads.compareValues(arr[blockPos + leftIndex * regBlockLen], 
	...
	public void sqrtCommonSort(int[] arr, int pos, int len, int[] extBuf, int extBufPos, boolean auxwrite) {
		...
		int blockLen = 1;
		for (; blockLen * 2 < len; blockLen *= 2);
		
		Writes.arraycopy(arr, pos, extBuf, extBufPos, blockLen, 1.0D, true, auxwrite);
		
		sqrtCommonSort(extBuf, extBufPos, blockLen, arr, pos, !auxwrite);
		...
		while (len > (buildLen *= 2)) {
			sqrtCombineBlocks(arr, pos + blockLen, len - blockLen, buildLen, blockLen, auxwrite);
		}
	...
	public void runSort(int[] array, int len, int bucketCount) {
		...
		for (; bufferLen * 2 < len; bufferLen *= 2);
		
		int[] extBuf = Writes.createExternalArray(bufferLen);
		
		sqrtCommonSort(array, 0, len, extBuf, 0, false);
