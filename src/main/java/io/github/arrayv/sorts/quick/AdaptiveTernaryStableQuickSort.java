...
public class AdaptiveTernaryStableQuickSort extends Sort {
    public AdaptiveTernaryStableQuickSort(ArrayVisualizer arrayVisualizer) {
        ...
        this.setAuthors("Flanlaina, Scandum");
    ...
    /**
     * Does a ternary partition around {@code piv} on the range {@code [a, b)} of
     * {@code array}, such that (E=array elements):
     * <p>
     * {@code [E < piv][E == piv][E > piv]}.
     * <p>
     * This method preserves the relative order of equal elements.
     * 
     * @param array the array
     * @param buf   the buffer. Its size must be at least {@code b - a}.
     * @param a     the start of the range, inclusive
     * @param b     the end of the range, exclusive
     * @param piv   the pivot to partition
     * @return An {@code int[]} with three values:
     *         <ul>
     *         <li>{@code [0]}: the start of the {@code [E == piv]} partition,
     *         inclusive.</li>
     *         <li>{@code [1]}: the end of the {@code [E == piv]} partition,
     *         exclusive.</li>
     *         <li>{@code [2]}: {@code 1} if the list is already correctly
     *         partitioned around {@code piv}, {@code 0} otherwise.</li>
     *         </ul>
     */
    protected int[] partition(int[] array, int[] buf, int a, int b, int piv) {
        Highlights.clearMark(2);
        // determines which elements do not need to be moved (this causes the eqSize ==
        // b - a check below to mean "is the range already partitioned?", not "does the
        // range contain only 1 unique element?")
        // I figured this out because Lazy Logsort with the bound reduction and all
        // equal check but no PDQsort's partial insert failed to sort on some Scrambled
        // Tail inputs.
        for (; a < b; a++) {
        ...
        return new int[] { p0, p0 + eqSize, (eqSize == b - a) ? 1 : 0 };
    }
    ...
    /**
     * Sorts the range {@code [a, b)} of {@code array} using an Adaptive Ternary Stable Quicksort.
     * 
     * @param array the array
     * @param a     the start of the range, inclusive
     * @param b     the end of the range, exclusive
