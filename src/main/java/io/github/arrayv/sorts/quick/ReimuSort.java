...
public final class ReimuSort extends Sort {
    ...
    void consumePartition(int[] array, PriorityQueue<Partition> queue, int a, int b, boolean bad) {
        if (b - a > INSERT_THRESHOLD) queue.offer(new Partition(a, b, bad));
        else insertSort(array, a, b);
    }

    protected void sortHelper(int[] array, int[] buf, int[] tags, int l, int r, int bLen) {
        if (r - l <= INSERT_THRESHOLD) {
            insertSort(array, l, r);
            return;
        }
        PriorityQueue<Partition> queue = new PriorityQueue<>((r - l - 1) / INSERT_THRESHOLD + 1);
    ...
    /**
     * Sorts the range {@code [a, b)} of {@code array} using Reimu Sort.
     *
    ...
    public void quickSort(int[] array, int a, int b) {
        ...
        if (z + e == len - 1) {
            ...
            if (e > 0) segmentReversal(array, a, b - 1, 0.5, 0.75, true, false);
            return;
        }
        int bLen = 1;
